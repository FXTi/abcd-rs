//! Hand-built module scaffolding for unit tests (mirrors the
//! `abcd-ir::verify` test builders; no production builder exists).
//!
//! Every helper keeps the IR invariants the verifier checks: blocks get
//! terminators, results are wired to `ValueDef::Inst`, and CFG edges are
//! recorded on the successor side (`link` / `add_try`).
...
/// A fresh function with one (entry) block, wired into class 0.
...
/// A `Const::Undefined` value (entry-defined).
pub fn const_undefined(m: &mut Module) -> ValueId {
    let c: ConstId = m.consts.push(Const::Undefined);
    let val = ValueId::new(m.values.len() as u32);
    m.values.push(Value {
