//! N54 regression (v0.1 lifter): `deprecated.defineclasswithbuffer
...
//! succeeded with the corrupted roles); post-fix it errs.
...
/// with the method_id operand wired to `f` itself (pre-fix the arm
/// resolved it and succeeded silently).
fn build_file() -> File {
...
fn deprecated_defineclasswithbuffer_is_hard_error() {
    ...
    assert!(
        matches!(
            err,
            abcd_ir::lift::LiftError::UnsupportedDeprecatedDefineClassWithBuffer
        ),
