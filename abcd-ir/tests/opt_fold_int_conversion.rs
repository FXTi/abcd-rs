//! N42 regression (P4-T3): both fold engines (peephole and SCCP) converted
//! f64 constants to integers with Rust's `as` casts, which SATURATE
//! (out-of-range clamps to ±2^31 / 0, NaN → 0 for `as i32` but the shift
//! count went through `as u32` which clamps negatives to 0). JS bitwise and
//! shift operators use ECMA-262 ToInt32/ToUint32 instead: WRAP mod 2^32,
//! with NaN/±Infinity → +0.
//!
...
//!
//! Red probes (P4-T2 review, verified): `2147483648|0` folded to
//! 2147483647 (JS: -2147483648); `1 << -1` folded to 1 (JS: -2147483648);
//! `Infinity|0` folded to 2147483647 (JS: 0); `~4294967296` folded to
//! -2147483648 (JS: -1).
...
/// Build `f() { return ~<lit> }` (Bytecode::Not is bitwise, N39).
fn build_bitnot_module(lit: f64) -> (Module, abcd_ir::entity::FuncId, abcd_ir::entity::Inst) {
    let mut module = Module::new(Version::new(12, 0, 6, 0), FileType::Dynamic);
    let func = IRBuilder::create_function(&mut module, "f", FunctionKind::Function, 0);
    let inst;
    {
        let mut builder = IRBuilder::new(&mut module, func);
        let operand = builder.emit_val(InstData::LiteralNumber(lit), IrType::default());
        let (unop_inst, result) = builder.emit(
            InstData::UnaryOp {
                op: UnOp::BitNot,
                operand,
            },
...
fn fold_binop<P: FuncPass>(pass: P, op: BinOp, acc_lit: f64, reg_lit: f64) -> f64 {
    ...
    match module.inst(inst).data {
        ...
        ref other => panic!("expected LiteralNumber fold, got {other:?}"),
    }
}
