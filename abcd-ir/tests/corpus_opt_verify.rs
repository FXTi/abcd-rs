...
use abcd_ir::{lift::lift_file, opt::optimize_module, verify::verify_module};

#[test]
#[ignore = "requires exported GHCR corpus"]
fn exported_corpus_optimization_preserves_structure() {
    ...
    for (line_no, line) in manifest.lines().enumerate() {
        ...
        let mut module = lift_file(&file).unwrap_or_else(|e| panic!("lift {relative}: {e}"));
        assert!(
            verify_module(&module).is_empty(),
            "pre-opt verify {relative}"
        );
