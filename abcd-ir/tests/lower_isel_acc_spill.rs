...
//! o->Acc, v->R1, spill slot -> R2. The bytecodes produced by the public
//! `isel::select` are executed on a tiny deterministic interpreter with the
//! accumulator seeded to an OBJ sentinel: the `Stobjbyvalue` object operand
//! must be OBJ.
...
fn acc_spill_before_ensure_acc_captures_object_not_key() {
    ...
    // Hand-pinned allocation: key in R0, object in the accumulator, value in
    // R1, and the reserved in-frame spill slot R2 (as regalloc would reserve
    // it: `num_regs` before reservation, then bumped to 3).
    let alloc = RegAlloc {
    ...
    // in-frame register R2 is emitted BEFORE the key's Lda (acc clobber),
    // and Stobjbyvalue reads the spill register.
    assert!(
        matches!(codes.first(), Some(Bytecode::Sta(Reg(2)))),
        "expected val_reg(object) = Sta(reserved spill R2) first, got {codes:?}"
    );
    assert!(
        matches!(codes.get(1), Some(Bytecode::Lda(Reg(0)))),
