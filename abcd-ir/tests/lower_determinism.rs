//! Opt-in determinism harness (N20): the lower → MethodBody → encode path
//! must be byte-reproducible. Two independent process runs of the corpus
//! rewrite historically produced hundreds of byte-different .abc files
//! (clustered in catchall-containing families), which poisons every
//! corpus-delta measurement.
//!
...
//! manifest-relative abc path).
//!
//! Run:
//!
//! ```text
//! cargo test -p abcd-ir --test lower_determinism --offline -- --ignored --nocapture
...
/// Read → decode → lift → verify (same front end as corpus_lower_oracle).
...
fn pinpoint(module_a: &Module, module_b: &Module, file: &File) -> String {
    ...
    for index in 0..module_a.functions.len().min(module_b.functions.len()) {
        ...
        let name = module_a
            .strings
            .get(module_a.func(func_id).name)
            .to_string();
        let (Ok(la), Ok(lb)) = (
...
"#,
    ...
    for relative in paths.lines() {
        ...
        // Opt variant: independent optimize_module runs on the two modules
        // must also converge byte-exactly (covers opt-pass nondeterminism).
        let mut opt1 = module1.clone();
        abcd_ir::opt::optimize_module(&mut opt1);
        let mut opt2 = module2.clone();
        abcd_ir::opt::optimize_module(&mut opt2);
        if !verify_module(&opt1).is_empty() || !verify_module(&opt2).is_empty() {
            continue;
        }
        if let (Some(oa), Some(ob)) = (
            rewrite_fixture(&opt1, &file1),
            rewrite_fixture(&opt2, &file2),
        ) {
            if oa != ob {
                e2e_mismatch += 1;
                eprintln!(
                    "E2E-NONDET(opt) {relative} ({} vs {} bytes)",
                    oa.len(),
                    ob.len()
                );
                eprint!("{}", pinpoint(&opt1, &opt2, &file1));
            }
        }
    }
