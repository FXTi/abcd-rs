...
//! rung 0 ([`Rung0AliasOracle`], the trivial baseline) or rung 1
//! ([`Rung1AliasOracle`], the demand-driven backward points-to engine),
//! selected by [`crate::driver::TaintConfig::alias_rung`].
//!
//! ## What the rung-1 arm changes for the flow functions
//!
...
//!   sound; see `abcd_analysis::dataflow::alias` module docs).
...
//!   method stays for key-level oracles (rung 0: empty) and rung-2
...
impl<'m> Oracle<'m> {
    /// The rung-0 local def-chain resolution (identical on both arms —
    ...
    /// The point-aware resolution: rung 1 refines through the memoized
    /// interprocedural query; rung 0 (and any imprecise query) answers
    /// with the local def chain.
    ...
    /// The rung-1 capability probe (§5.2): allocation sites of a base
    ...
    /// precise; the caller skips its baseline (rung-0) re-key then.
    /// Rung 0 injects nothing (aliasing is resolved at the fact key).
    pub fn aliases_of_store(
        ...
        match self {
            Oracle::Rung0(_) => Vec::new(),
            Oracle::Rung1(o) => {
                let ans = o.query(object, store);
...
/// oracle-agnostic (the rung-1 engine's docs apply; rung 0 is the
/// baseline behavior).
