...
//! there is no VM evidence for the lift — the v0.2 lifter must refuse
//! loudly, mirroring v0.1 (maintainer ruling 2026-09-20, N51: hard
//! error, never a warning, never silent pass-through).
...
fn build_byvalue(make: impl FnOnce() -> Bytecode) -> abcd_file::File {
...
fn ldthisbyname_is_hard_error() {
    ...
    assert!(
        matches!(
            err,
            abcd_lift::LiftError::UnsupportedThisByAccess("ldthisbyname")
        ),
...
fn stthisbyname_is_hard_error() {
    ...
    assert!(
        matches!(
            err,
            abcd_lift::LiftError::UnsupportedThisByAccess("stthisbyname")
        ),
        "dedicated N51 variant naming the opcode, got: {err:?}"
    );
    let msg = err.to_string();
...
fn ldthisbyvalue_is_hard_error() {
    ...
    assert!(
        matches!(
            err,
            abcd_lift::LiftError::UnsupportedThisByAccess("ldthisbyvalue")
        ),
        "dedicated N51 variant naming the opcode, got: {err:?}"
    );
    let msg = err.to_string();
...
fn stthisbyvalue_is_hard_error() {
    ...
    assert!(
        matches!(
            err,
            abcd_lift::LiftError::UnsupportedThisByAccess("stthisbyvalue")
        ),
        "dedicated N51 variant naming the opcode, got: {err:?}"
    );
    let msg = err.to_string();
