...
# vendor-sync.rb — Sync vendored arkcompiler files from upstream
#
# Usage: ruby <crate>/vendor-sync.rb [options]
...
_config = File.join(__dir__, 'vendor-sync-files.yml')
abort "ERROR: #{_config} not found" unless File.exist?(_config)
FILE_MAP = YAML.safe_load_file(_config).freeze
