...
**维护者决策（2026-09-25，测试质量评估后）**：① **桥接 C++ 不进 90% 覆盖率分母**（D3 延伸：死 FFI 面保留但不计量；90% 目标口径 = 我们的 Rust、含语料全量——nightly coverage-true artifact，当前地板 84.38% 已经 orchestrator 复现）；② 委托桥接死代码详析（量化可删面 + 证明厂商必要逻辑已全部经活导出可达 → q-P1）；③ 批准六个廉价弱测试修复（W1/W4/W5/W6/W8/W9 → q-P2；红先行纪律：W5 的 hits==0 与 W8 的零-skip 门须先验证不变量当前成立方可落地）。
