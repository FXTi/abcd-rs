...
| q-P3 | 厂商知识隔离审计（q-P1 续） | ①file-sys/isa-sys 是否把可随版本变动的厂商知识完整隔离在桥接层（桥函数包装 + bindgen 直接引用上游常量）；Rust 侧有无硬编码的厂商派生常量/布局/偏移（绕桥的泄漏）；v7.0 repin（PR #20）实测破损清单=泄漏实证 ②同一 vendor 能力多路径时（厂商 accessor TU/头内联/手工重读布局/拷贝常量）选择是否最优（抗版本变动 × 性能） | worker q-P3 (k3) | **进行中** |
