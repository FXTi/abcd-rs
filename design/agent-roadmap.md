...
| v2-P4 | 替换：abcd-ir **删除**（不留档，维护者 2026-09-21 拍板；git 历史即留档），abcd-ir2 正名 abcd-ir；parity 对照器/v0.1 语料 driver 一并退役 | worker v2-P4 (k3) | **进行中**（2026-09-21 启动；四阶段：退役 parity 对照器（保留 lift+verify 门禁为 corpus_lift_verify）→ 删 abcd-ir → abcd-ir2 改名 → 文档；门禁=三变体对验收基线 0 差异 + v2opt oracle 重跑 + 全套件绿） |
