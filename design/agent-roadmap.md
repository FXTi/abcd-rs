...
| N68 | **G6**：现代 async 族字节码（asyncfunctionawaituncaught/resolve/reject）的值在 **acc** 传递（isa.yaml `acc:inout:top`；runtime interpreter-inl.cpp ASYNCFUNCTIONAWAITUNCAUGHT_V8），但 abcd-lift 只建 funcobj 寄存器操作数——值根本不进 IR，异步函数体的 sound 反编译 fold 不存在（d-P11 发现；异步 fixture 门禁 not-applicable 故梦想门禁零成本）。修复=lift 加 acc 操作数（同族 N65/N66/N67 的 acc 语义缺席形态第四例） | 🔲 登记（2026-09-25，d-P11 报告并记入 design/decompile.md G6；后续批次候选，届时需要 abcd-lift 解冻 + 异步 fixture 语料/探针——当前语料异步族仅 runtime-not-applicable 覆盖） |
...
| d-P12 | N68/G6 异步 acc 值缺口 | abcd-lift 现代 async 族（awaituncaught/resolve/reject）改读 acc 为值、v0=funcobj（vendor interpreter-inl.cpp:5357-5366 实证）+ lower 字节中性证明 + abcd-decompile 异步 fold（AsyncResolve→return/AsyncReject→throw/AwaitUncaught→await）；证据层：字节恒等 + pandasm 逐指令 + node --check/行为（VM 对异步 not-applicable，dream gate 重编译须零语法错） | worker d-P12 (k3) | **进行中**（2026-09-25 维护者拍板做；同族第四例 acc 语义缺席） |
