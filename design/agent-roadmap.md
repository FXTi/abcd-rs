...
| N8 | typeSummaryOffset 是否偏移存疑 | ✅ 已裁决（P5-T2，登记为潜在悬挂类但不修）：**是文件偏移**——changelog 2022-08-18-isa-changelog.md 第 5 条 "The literalarray which is referenced by TypeSummary contains the offsets of all type-literalarray"（且是**嵌套**偏移：指向的 LA 元素本身又是偏移）。但：vendored 树无生产者（es2panda emitter 不发射）、无运行时消费者（TYPE_SUMMARY_OFFSET_NOT_FOUND=0 是死常量，js_pandafile.h:98 仅此一处）、disassembler 显式排除（disassembler.cpp:1009 不入 module_literals_）、语料零 fixture。若未来出现：当前 decode 落 FieldValue::I32 裸值 → 重写悬挂。修复 sketch：同 N7 模式按字段名匹配 + 嵌套偏移需二级重定位（外层 blob 偏移 + 内层各 type-LA 偏移），工作量超 N7；待真实触发样本再做 |
...
| v2-P0.5 | abcd-ir2 分类表扩到 ISA 全覆盖（P1 前置：14+8 个真 op + payload homes——orchestrator 裁决：拒绝占位/有损映射，三条语义错误（SetObjectWithProto≠CopyDataProps、LoadNewTarget≠undefined、GetTemplateObject≠Mov）） | worker v2-P1 (k3) | **进行中** |
