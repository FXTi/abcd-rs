...
| c-P6 | test262 P1–P3 深化 | P1 pandasm 扩展（**已事实上落地**：file-isa 套件迭代全部 5517 行含 test262，4,557,285 指令零 mismatch，per-push 在 CI）；P2 VM oracle 覆盖 test262 2685 行（新套件 + 新 CI job，stderr 归一化政策走实测分诊）；P3 dream-gate 抽样（待 P2 落地后启动，先做 report-only 仪表，分诊后转门） | worker c-P6 (k3) | **进行中（P2）** |
| N71 | **test262 首触实证**：24/2685 fixture v2lift 重写 encode 失败（operand out of range）——大帧函数（>256 vreg）的 ≥256 vreg 被编进 8 位寄存器-立即数槽，而这些指令在 isa.yaml **无 wide 形**（callthis0/2/3、strictnoteq、neg、shr2、add2、ashr2、tonumber、callarg1；S2(P3-T6) 修复面只覆盖 callrange 系 wide 选择 + sta/lda 低位 scratch，残留此族） | 修复=isel 对无 wide 形指令的高位 vreg 经低位 scratch 中转（P3-T6 手法）；证据层：24 fixture 重写通过（红先行）+ 现有 1149×3 重写字节零差异 + pandasm 455 万零 mismatch + 梦想门禁 1149/1149 不动 | worker l-P1 (k3) | **进行中** |
