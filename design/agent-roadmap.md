...
| q-P2 | 六弱测试修复 | W1 version 断言 / W4 探针命中行号 / W5 hits==0 门（先验当前零命中）/ W6 inline 确定性默认开 / W8 零-skip 门（先验当前零 skip）/ W9 spawn-to-probe + node --check 转致命 | worker q-P2 (k3) 中途断线 → **orchestrator 接手完成** | **完成**（a7c3be1/92e342f/ca0c93d/32c0ae4/9316a1f；worker 断线时树内零改动，全部修复由 orchestrator 重做。验证（dabai 单批次）：fmt 净、workspace 绿、TAINT-FLOWS hits=0（W5 不变量实测成立+落门）、lower oracle 1149/1149/1149 skipped 0×3（W8 不变量成立+落门；W6 默认开后套件 1.40s→1.76s）、NODE-CHECK ok=40 bad=0（W9 spawn 探测+致命门）。W9 的 TS 段保持非致命（依赖 node 版本实验 API，注释注明）；W4 两个 evaluate 调用点补标记行号 &[10]/&[30]；W1 钉 Some+major==12（vendor 模板 beta1=默认子版本实证）） |
| q-P3 | 厂商知识隔离审计（q-P1 续） | ①file-sys/isa-sys 是否把可随版本变动的厂商知识完整隔离在桥接层（桥函数包装 + bindgen 直接引用上游常量）；Rust 侧有无硬编码的厂商派生常量/布局/偏移（绕桥的泄漏）；v7.0 repin（PR #20）实测破损清单=泄漏实证 ②同一 vendor 能力多路径时（厂商 accessor TU/头内联/手工重读布局/拷贝常量）选择是否最优（抗版本变动 × 性能） | worker q-P3 (k3) | **完成**（design/bridge-vendor-isolation.md；**结论：隔离架构成立且经 v7.0 repin 实证**——唯一实测泄漏在桥内（GetFileType 引 master 专属常量，编译期响亮报错，Rust 零改动）；FFI 边界只传不透明句柄+原生类型+自有 POD。泄漏：V-I1 四个 vendor 名称字符串（decode.rs:549-576，vendor 树内有同名常量可钉）、V-I2 frame.rs callType 位布局（源在 ets_runtime，结构性不可钉=接受+文档）、V-I3 module.rs 段落顺序（pandasm 路径，测试钉住）。路径评审：tolerant 枚举/MethodHandle 手工/IndexAccessor 旁路/GetFileType 移植/bindgen 常量面全部最优或必然最优；唯一次优=V-I7（ISA 分类逐条 FFI 在 lift 热路径，可改生成 Rust 常量表，性能论断未实测——先测量）。下次 repin 前行动：钉 V-I1、diff ParamAnnotationsItem 构造器（V-I4 有测试绊线）、先落 q-P1 删除缩编译面、刷新 vendor-sync.md §3（V-I8 文档过时）。**orchestrator 独立复验**：V-I1/V-I2/V-I4/V-I7/repin 提交链/STATIC_VERSION/isa.yaml 版本代差逐项复现） |
| q-P4 | repin 前行动包（删除+V-I1+V-I8） | q-P1 死面删除 100 导出 + V-I1 四字符串钉住 + V-I8 vendor-sync.md 刷新（V-I4 入 repin 检查单） | orchestrator DIY（worker 断线后不另派） | **完成**（bd62e6a/0d566f0/46f4a3f/3c16944；删除经三轮 span 检测返工（strip() 缩进陷阱、catch 闭合行陷阱——均以基线大括号配平+活导出逐一核对卡住）；契约注释与 AbcEntityIdCb/AbcProtoTypeExCb 孤儿 typedef 随清。V-I1：3/4 字符串经桥接 abc_vendor_* 导出钉住（ark::TYPE_SUMMARY_FIELD_NAME/MODULE_REQUEST_PAHSE_IDX/SCOPE_NAME_RECORD），L_ESModuleRecord; 无可引用 vendor 常量（cone 内私有/cone 外公有）=行为钉+注释。验证（dabai 单批）：fmt 净、build 净、**workspace 734 passed/0 failed/0 error**（基线 731+3 新钉）、corpus_lift_verify/real_module_abc+pandasm/corpus_lower_oracle 全绿、1149×3 skipped 0。V-I4 已写入 vendor-sync.md §3 repin 检查单） |
| c-P1 | 测试结构迁移（根包+套件归位） | 根 package `abcd-rs`（package+workspace 合一）+ 28 个跨 crate 套件按数据流命名迁入根 tests/（维护者布局裁决） | worker c-P1 (k3) | **完成**（63d3bc2；7 目标 file-isa/file-lift/file-sys-file/lift-lower/lift-analysis/lift-taint/lift-decompile，20 文件 git rename，套件内容零改动；两处 edition-2024 let-chain 改 tuple if-let；common 去重（两套 IR 脚手架语义分歧逐字保留）。**orchestrator 独立复验**：fmt 净（远程）、workspace 734/0/31 与迁移前基线逐字一致、corpus_lift_verify 2787/12996/0 新位置复现、**梦想门禁 1149/1149 全桶零新位置本地自跑（287s）**、lift-lower/lift-taint 抽查绿） |
| c-P2 | 镜像侧改造（arkcompiler-test） | A1 generate 并行化 / A2 test262 P0 预构建 / A3 探针+yield-star 收编 / A4 gitee manifest 雷达 / A5 README 人工发布流程 | worker c-P2 (k3) → orchestrator 复审合并 | **完成**（arkcompiler-test@91e78c7 merge 入 main；A1 并行化串行逐字节一致证明（41.4s→3.3s/12.5×，5712 fixtures 8.7s）；A2 分桶：compiled 5487/es2abc-cant 1/expected-negative-parse 224(agreement 224/224)/skipped-includes 801/flags 177/async 47/missing 18/module 10，es2abc 成功率 92.3%；A3 +45（40 探针+5 yield-star，24.0.0.0/baseline 结构性）；A4 雷达=gitee manifest 仓 branch+tag 通配按 committer date（该仓 OpenHarmony-* 是分支非 tag）；runtime_checked 保持 1119 不污染。**orchestrator 独立复验**：逐 commit diff 复审 + 镜像 info 桶计数复核 + test262/负例/yield-star 行抽查。README 补 3968→3963 去重注。make build 42.5s / make test 28.6s 全绿（worker dabai 实测））**待维护者：dabai 上 make build && make test && make push 出新 digest** |
| c-P3 | abcd-rs 语料切换 + CI 七 job | 测试数据零内置（decompile-fixtures/probes-taint 删除、套件改读 export）+ 计数重基线 + test262 lift/verify 门 + per-push CI 图 + coverage 全量化 | worker c-P3 (k3) → orchestrator 复验落盘 | **完成**（f0e1252/a711300/b9dcfbc/2efd38f/7b56a94；计数：lift 2787→5517（2832+2685 拆分、45592 函数、pandasm 4,557,285 指令零 mismatch 互证）、decompile 系 2832（test262 滤除留后期）、分析三套件 5517、runtime-passed 1149 不变；直方图预期移动（yield_star 0→4、for_await_of 0→1 首次语料内触发）；过程修复三件测试基建（pandasm raw 字符串 oracle 消歧+有界前瞻、es2abc-24 新字面量标签映射、有损浮点打印调和）+ 新登记孤 surrogate 有损类（钉 4 fixture/8 串）+ merge_stats 漏字段 harness 修复；CI 七 job 全 per-push 无 nightly、digest 钉 ARK_TEST_IMAGE。**orchestrator 独立复验**：远程 fmt 净/workspace 720/0/45（-14/+14=yield-star 转语料依赖，已解释）、file-lift 5517/45592/0 复现、pandasm 4,557,285/0 复现、探针 tp=32 fp=1 fn=0 复现、smoke hits=0 复现、**梦想门禁本地自跑 1149/1149 全桶零（222s）**。**流程事故登记**：e5afe62（docs commit）把 worker 已暂存的 Part-1 删除/迁移卷入（git commit 提交整个 index；内容无误但标签错——共享 index 并行作业教训：落盘前必查 git status 暂存区）） |
| c-P6 | test262 P1–P3 深化 | P1 pandasm 扩展（**已事实上落地**：file-isa 套件迭代全部 5517 行含 test262，4,557,285 指令零 mismatch，per-push 在 CI）；P2 VM oracle 覆盖 test262 2685 行（新套件 + 新 CI job，stderr 归一化政策走实测分诊）；P3 dream-gate 抽样（待 P2 落地后启动，先做 report-only 仪表，分诊后转门） | worker c-P6 (k3) | **P2 完成**（8d1ff4f：test262_vm 套件 + compare --recorded/error-name 归一/--expect-divergences + CI job test262-vm + 账簿 30 行=**N72**（维护者裁决 B 方案：带自清零账簿上岗）；身份基线 2659 精确+26 纯路径、exit/stdout/timeout 零分歧；门=pass+documented==total。P3 待启动） |
| N71 | **test262 首触实证**：24/2685 fixture v2lift 重写 encode 失败（operand out of range）——初诊"高位 vreg 入 8 位寄存器槽"被更正为 IC 槽溢出（见终态列），而这些指令在 isa.yaml **无 wide 形**（callthis0/2/3、strictnoteq、neg、shr2、add2、ashr2、tonumber、callarg1；S2(P3-T6) 修复面只覆盖 callrange 系 wide 选择 + sta/lda 低位 scratch，残留此族） | 修复=isel 对无 wide 形指令的高位 vreg 经低位 scratch 中转（P3-T6 手法）；证据层：24 fixture 重写通过（红先行）+ 现有 1149×3 重写字节零差异 + pandasm 455 万零 mismatch + 梦想门禁 1149/1149 不动 | worker l-P1 (k3) | **完成**（0f8c52d。**根因更正（vendor 实证）**：非 vreg 入槽（neg/tonumber 无寄存器操作数是铁证），真因=IC 槽 8 位立即数溢出——isel 单计数器密集分配，method 总 IC 槽 >256 时 eight_bit_ic（imm:u8 无宽形）溢出。修复=isel 后处理镜像上游 PandaGen::ReArrangeIc（8 位先排/16 位续后/耗尽降级 0xFF=INVALID_IC_SLOT 行为保持，仅溢出触发，非溢出函数字节零改动）+ 硬错误 IcSlotOverflow/IcOperandMapGap + 红先行 24/24。证据：1149×3 字节零差异、pandasm 455 万零 mismatch、梦想门禁 1149/1149、test262_vm 2685/2685 零 skip；修复解禁暴露第 30 条行为分歧（surrogate-pairs，入 N72 账簿）。orchestrator 独立复验全过） |
